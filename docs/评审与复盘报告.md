...
| VIP 系统 | 情绪低落时自动赠送的温暖关怀 |
| 守护者（Guardian） | 社区内容监管与灯火转赠 |
...
| 3 | API 文档自动生成 | 后端 197 个端点缺少 OpenAPI/Swagger 文档 | 后端 |
...
1. **微服务拆分预案**：当前单体后端承载 20 个 Controller、197 个端点。当 QPS 增长到一定规模时，可考虑将 AI 推荐引擎、消息系统拆分为独立服务。
...
- **架构层面**：严格遵循 DDD 四层架构，ServiceLocator 实现线程安全的依赖注入，EventBus 实现领域事件解耦。20 个 Controller 覆盖 197 个 API 端点，11 个数据库迁移脚本保证 schema 演进可追溯。
