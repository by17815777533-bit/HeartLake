...
> 本文档覆盖 HeartLake 后端全部 20 个 Controller、180+ 个 API 端点。
> 基于 `backend/include/interfaces/api/*.h` 源码自动整理，最后更新：2026-02-23。
...
守护者系统提供灯火转赠与情绪洞察功能。所有端点均需 SecurityAuditFilter 认证。

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/guardian/stats` | 获取守护者统计数据 |
| GET | `/api/guardian` | 获取守护者统计数据（别名） |
...
| POST | `/api/edge-ai/emotion-sample` | 提交情绪样本（脉搏更新） |
...
`/api/edge-ai/vector-search` 支持两种输入：
- `query/text`（服务端实时生成 embedding）
- `vector`（预计算向量）

可选参数：
- `candidate_multiplier`：第一阶段召回倍率（默认 4，范围 1-8）
- `enable_second_stage`：是否启用二阶段精排（默认 `true`）
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `avg_score` | float | 原始情绪均值（`-1` 到 `1`） |
| `normalized_score` | float | 归一化情绪均值（`0` 到 `1`） |
| `trend` | string | `rising`/`stable`/`falling` |
| `trend_slope` | float | 线性趋势斜率 |
| `sample_count` | int | 窗口样本数 |

**情绪样本提交请求体（`POST /api/edge-ai/emotion-sample`）：**
...
`score_scale` 可选值：
- `signed`（默认，`-1~1`）
- `zero_one`（兼容旧端，服务端会自动转换为 `-1~1`）

`confidence` 为可选字段（`0~1`），用于低置信度样本降权，提升情绪脉搏稳定性；不传时默认 `1.0`。

`mood` 建议使用：
`happy|calm|neutral|anxious|sad|angry|surprised|confused|hopeful|grateful|lonely`。  
服务端会对常见别名做归一化，非法值回退为 `neutral`。
...
**ONNX 小模型启用说明（EdgeAI 本地情感分析）：**

- 需要后端编译时开启 `HEARTLAKE_USE_ONNX=ON`。
- `EDGE_AI_ONNX_ENABLED` 支持：
  - `true` / `1` / `on`：强制尝试 ONNX
  - `false` / `0` / `off`：禁用 ONNX
  - 未设置：自动探测模型与词表文件，存在即自动启用
- `EDGE_AI_MODEL_PATH` 可填模型文件路径（如 `./models/sentiment_zh.onnx`）或目录（如 `./models`）。
- `EDGE_AI_VOCAB_PATH` 可选；不填时默认使用模型目录下 `vocab.txt`。
- 管理接口 `GET /api/admin/edge-ai/config` 会返回当前 `onnx_enabled/model_path/vocab_path/onnx_threads/onnx_compiled`。
...
| 守护者与咨询 | GuardianController | 5 |
| 守护者与咨询 | ConsultationController | 5 |
