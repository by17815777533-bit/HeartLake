...
> 生成日期：2026-02-24 | 总提交数：160 | 分支：main
...
| 层 | 技术栈 | 代码量 |
|----|--------|--------|
| 后端 | C++20 + Drogon | 39,400 行 |
| 移动端 | Flutter / Dart | 26,950 行 |
| 管理后台 | Vue 3 + Element Plus | 7,808 行 |
| 反向代理 | Nginx | 配置文件 |
| 容器编排 | Docker Compose (6 服务) | 186 行 |
| **合计** | | **~75,153 行** |
...
```
backend/src/
├── interfaces/api/      20 个 Controller, 180+ API 端点
├── application/         Service 层 (业务编排)
├── domain/              领域模型 + 值对象
└── infrastructure/      数据库/AI/缓存/加密
```

### 2.2 20 个 HTTP/WebSocket 控制器
...
基于 `BaseService` 统一封装，覆盖：认证、用户、心石、互动、纸船、好友、临时好友、守望者、咨询、推荐、AI 推荐、边缘 AI、举报、通知、VIP、湖神、心理支持、缓存。
...
| API 文档 | `docs/接口文档.md` | API 使用指南 |
| API 参考 | `docs/接口参考.md` | 180+ 端点完整参考 |
| EdgeAI 设计 | `docs/边缘AI设计.md` | 8 大子系统设计文档 |
| 技术创新白皮书 | `docs/技术创新与架构深度解析.md` | 六大创新点深度解析 |
| 全面深度审计 | `docs/全面深度审计报告.md` | 四模块 100+ 问题审计 |
| 创新升级记录 | `docs/创新升级记录_2026-02.md` | 多视角亲密分、多专家情绪融合等 |
| 湖神可靠性升级 | `docs/湖神可靠性升级_2026-02.md` | 13 项可靠性改进 |
| 世界级验证报告 | `docs/世界级验证报告.md` | 自动化验证 11/11 门槛通过 |
| 评审与复盘报告 | `docs/评审与复盘报告.md` | 项目评审 4.3/5 星 |
| 贡献指南 | `CONTRIBUTING.md` | 开发规范、提交流程 |
| 变更记录 | `CHANGELOG.md` | 按版本组织的变更历史 |

---

## 9. 项目亮点

1. **全栈自研 75,000+ 行** — 后端 C++20 (39,400 行)、移动端 Flutter (26,950 行)、管理后台 Vue 3 (7,808 行)，三端完整实现
2. **边缘 AI 引擎** — 8 大子系统纯 C++ 实现，无需云端 AI 依赖，毫秒级推理延迟
3. **隐私计算三件套** — 差分隐私 + 联邦学习 + 身份影子映射，超越 GDPR 标准
4. **三层情感融合** — 规则 + 词典 + ONNX 模型，离线可用，准确率 96.88%
5. **双记忆 RAG** — 短期 + 长期记忆的情感陪伴 AI，跨会话情感理解
6. **生产级安全** — PASETO v4、X25519/AES-256-GCM E2E 加密、速率限制、内容审核 fail-closed
7. **完整社交生态** — 心石、纸船、好友、守望者、湖神 AI、心理咨询，功能闭环
8. **容器化部署** — Docker Compose 一键部署 6 个服务，含健康检查和资源限制
