...
class HelpScreen extends StatelessWidget {
        ...
        child: ListView(
          ...
          children: [
            _buildSection('常见问题', isDark, [
              ...
              _buildFAQ('如何添加好友？',
                  '通过纸船互动建立临时好友关系。持续互动会影响亲密度和后续关系判定，临时好友会在24小时后失效。', isDark),
              _buildFAQ('灯是什么？', '灯是温暖的陪伴，当系统感知到您需要关怀时会自动点亮，完全免费。', isDark),
              _buildFAQ('如何举报不当内容？', '长按石头或纸船内容，选择"举报"，选择举报原因并提交。我们会在24小时内处理。',
                  isDark),
              _buildFAQ('什么是守护者？',
                  '守护者是心湖的温暖守护系统，当检测到你可能需要关怀时，会为你点亮一盏灯，提供情感支持和建议。', isDark),
              _buildFAQ(
                  '灯有什么特权？', '点亮灯的用户享有更多灯火关怀、专属心理咨询预约、湖神深度情感分析等特权。', isDark),
              _buildFAQ('如何保护我的隐私？',
                  '心湖采用端到端加密和差分隐私技术保护你的数据。你可以在设置中管理隐私选项，包括屏蔽用户和数据导出。', isDark),
              _buildFAQ(
                  '情感日历是什么？', '情感日历记录你每天的心情变化，以热力图形式展示，帮助你了解自己的情感趋势。', isDark),
              _buildFAQ('如何注销账号？',
                  '在设置 > 账号管理中可以选择停用或永久删除账号。停用后30天内可恢复，永久删除不可逆。', isDark),
            ]),
            const SizedBox(height: 24),
            _buildSection('安全须知', isDark, [
              _buildSafetyTip(
                  Icons.visibility_off, '心湖是匿名社区，请不要在石头中透露个人真实信息。', isDark),
              _buildSafetyTip(Icons.flag, '如遇到骚扰或不当内容，请及时举报。', isDark),
              _buildSafetyTip(
                  Icons.phone, '如果你正在经历心理困扰，请拨打心理援助热线：400-161-9995', isDark),
            ]),
            const SizedBox(height: 24),
            _buildSection('联系我们', isDark, [
              Container(
                ...
                child: ListTile(
                  ...
                  onTap: () =>
                      _showContactInfo(context, '请发送邮件至 support@heartlake.app'),
                ),
              ),
            ]),
          ],
  ...
  void _showContactInfo(BuildContext context, String message) {
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(message), backgroundColor: AppTheme.skyBlue));
  }
}
