...
struct EmotionMemory {
  ...
  std::vector<ShortTermEntry>
      shortTerm; ///< 短期记忆队列，最多 MAX_SHORT_TERM 条
  ...
  struct LongTermProfile {
    ...
    std::string emotionTrend =
        "stable"; ///< 情绪趋势: rising / falling / stable
...
public:
   ...
   * @return AI生成的个性化回复
   */
  std::string generateResponse(const std::string &userId,
                               const std::string &currentContent,
                               const std::string &currentEmotion,
                               float emotionScore);

  RagReplyResult generateResponseResult(const std::string &userId,
                                        const std::string &currentContent,
                                        const std::string &currentEmotion,
                                        float emotionScore);
   ...
   * 更新用户的短期记忆
   */
  void updateShortTermMemory(const std::string &userId,
   ...
   */
  std::string
  buildRAGPrompt(const EmotionMemory &memory, const std::string &currentContent,
                 const std::string &currentEmotion,
...
private:
  ...
  static constexpr int LONG_TERM_REFRESH_INTERVAL_SECONDS =
      180; ///< 长期画像最小刷新间隔
   ...
   */
  void refreshLongTermMemoryImpl(const std::string &userId, bool forceRefresh,
                                 double nowEpoch);
   ...
   */
  void
  updateShortTermMemoryInternal(const std::string &userId,
                                const std::string &content,
                                const std::string &emotion, float score,
   ...
   */
  bool shouldRefreshLongTermMemory(const std::string &userId,
                                   double nowEpoch) const;

  /**
   * @brief 记录被命中的短期记忆访问统计
   */
  void markShortTermEntriesAccessed(const std::string &userId,
   ...
   */
  void evictLeastRelevant(std::vector<EmotionMemory::ShortTermEntry> &entries,
                          const std::string &currentContent,
                          const std::string &currentEmotion,
