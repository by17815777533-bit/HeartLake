 ...
 * 暖心语录服务实现
 *
 * 根据用户所处场景选择合适的暖心文案，覆盖 14 种触发场景：
 * 深夜独处、无人回应、持续负面、节日独处、生日独处、久别回归、
 * 首次投石、连续被忽略、情绪骤降、周末夜晚、回访 Day1/3/7、长期不活跃。
 *
 * 每个场景维护独立的语录池，通过随机采样避免重复感。
 * 高风险场景（持续负面、情绪骤降、连续被忽略）会附带 VIP 引导文案，
 * 提示用户可预约免费心理咨询。
 *
 * detectScene() 按优先级匹配场景：生日 > 节日 > 久别 > 首投 > 骤降 > 被忽略 > 深夜。
 */
