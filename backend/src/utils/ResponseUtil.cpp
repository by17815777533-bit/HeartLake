...
drogon::HttpResponsePtr ResponseUtil::created(const Json::Value& data, const std::string& message) {
    return createResponse(drogon::k201Created, 0, message, data);
}

drogon::HttpResponsePtr ResponseUtil::error(ErrorCode errorCode, const std::string& detail, const Json::Value& extra) {
    int httpStatus = ErrorCodeHelper::getHttpStatus(errorCode);
    std::string msg = detail.empty() ? ErrorCodeHelper::getMessageZh(errorCode) : detail;
    return createResponse(static_cast<drogon::HttpStatusCode>(httpStatus), static_cast<int>(errorCode), msg, extra);
}

drogon::HttpResponsePtr ResponseUtil::error(int httpStatus, const std::string& message, const Json::Value& data) {
    return createResponse(static_cast<drogon::HttpStatusCode>(httpStatus), httpStatus, message, data);
}

drogon::HttpResponsePtr ResponseUtil::paged(const Json::Value& items, int total, int page, int pageSize) {
    Json::Value data;
    data["items"] = items;
    data["total"] = total;
    data["page"] = page;
...
drogon::HttpResponsePtr ResponseUtil::tooManyRequests(const std::string& message) {
    return createResponse(drogon::k429TooManyRequests, 429, message);
}

drogon::HttpResponsePtr ResponseUtil::internalError(const std::string& message) {
    return createResponse(drogon::k500InternalServerError, 500, message);
}

drogon::HttpResponsePtr ResponseUtil::createResponse(
