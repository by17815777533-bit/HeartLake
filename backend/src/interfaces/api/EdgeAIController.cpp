...
void setMirroredConfig(Json::Value &config, const std::string &snakeKey,
                       const std::string &camelKey, const Json::Value &value) {
...
Json::Value buildAdminEdgeAIConfigPayload(const Json::Value &aiConfig,
                                          ...
                                          bool engineEnabled) {
  const bool edgeAiEnabled =
      aiConfig.isMember("edge_ai_enabled")
          ? parseJsonBoolValue(aiConfig["edge_ai_enabled"],
                               heartlake::utils::parseBoolEnv(
                                   std::getenv("EDGE_AI_ENABLED"), true))
          : heartlake::utils::parseBoolEnv(std::getenv("EDGE_AI_ENABLED"),
                                           true);
  const std::string onnxEnabled =
      aiConfig.isMember("onnx_enabled")
          ? parseJsonStringValue(aiConfig["onnx_enabled"], "auto")
          : parseJsonStringValue(
                Json::Value(std::getenv("EDGE_AI_ONNX_ENABLED")
  ...
  config["inferenceEngine"] = onnxEnabled == "false" ? "builtin" : "onnx";
  setMirroredConfig(
      config, "model_path", "modelPath",
      aiConfig.get("model_path", std::getenv("EDGE_AI_MODEL_PATH")
                                     ? std::getenv("EDGE_AI_MODEL_PATH")
                                     : "./models/sentiment_zh.onnx"));
  setMirroredConfig(
      config, "vocab_path", "vocabPath",
      aiConfig.get("vocab_path", std::getenv("EDGE_AI_VOCAB_PATH")
  ...
  setMirroredConfig(
      config, "hnsw_ef_search", "hnswEfSearch",
      aiConfig.get("hnsw_ef_search", engineStats.get("hnsw_ef_search", 50)));
  setMirroredConfig(
      config, "dp_epsilon", "dpEpsilon",
      aiConfig.get("dp_epsilon", engineStats.get("dp_epsilon", 1.0)));
  setMirroredConfig(
      config, "dp_delta", "dpDelta",
      aiConfig.get("dp_delta", engineStats.get("dp_delta", 1e-5)));
  setMirroredConfig(config, "pulse_window_seconds", "pulseWindowSeconds",
                    aiConfig.get("pulse_window_seconds",
                                 engineStats.get("pulse_window_seconds", 300)));
  setMirroredConfig(
      config, "sentiment_cache_ttl", "cacheTTL",
      aiConfig.get("sentiment_cache_ttl",
                   engineStats.get("sentiment_cache_ttl_seconds", 300)));
  setMirroredConfig(config, "sentiment_cache_max", "sentimentCacheMax",
  ...
  setMirroredConfig(
      config, "cache_strategy", "cacheStrategy",
  ...
  setMirroredConfig(
      config, "cache_size_mb", "cacheSizeMB",
      aiConfig.get("cacheSizeMB", aiConfig.get("cache_size_mb", 256)));
  setMirroredConfig(config, "federated_interval", "federatedInterval",
                    aiConfig.get("federatedInterval",
                                 aiConfig.get("federated_interval", 300)));
  setMirroredConfig(
      config, "emotion_model", "emotionModel",
      aiConfig.get("emotionModel", aiConfig.get("emotion_model", "standard")));
  setMirroredConfig(
      config, "vector_search_enabled", "vectorSearchEnabled",
  ...
  setMirroredConfig(
      config, "federated_enabled", "federatedEnabled",
      aiConfig.get("federatedEnabled",
                   aiConfig.get("federated_enabled", edgeAiEnabled)));
  setMirroredConfig(config, "inference_timeout", "inferenceTimeout",
                    aiConfig.get("inferenceTimeout",
                                 aiConfig.get("inference_timeout", 5000)));
...
double resolveCacheHitRatePercent(const Json::Value &cacheMetrics) {
  ...
  if (cacheMetrics.isMember("semantic_cache")) {
    hitRate =
        cacheMetrics["semantic_cache"].get("hit_rate", hitRate).asDouble();
  }
  if (hitRate <= 1.0) {
    hitRate *= 100.0;
  }
...
ConfidenceCalibration calibrateConfidence(float score, float confidence,
                                          size_t textLength) {
  const float margin = std::min(1.0f, std::abs(score));
  const float lengthFactor =
      textLength < 6 ? 0.82f : (textLength < 12 ? 0.90f : 1.0f);
  const float neutralPenalty = std::max(0.0f, 0.18f - margin) * 1.8f;
  const float calibrated = clampUnit(
      confidence * (0.60f + 0.40f * margin) * lengthFactor - neutralPenalty);
  const float uncertainty =
      clampUnit(1.0f - calibrated + std::max(0.0f, 0.16f - margin) * 1.2f);
  const bool abstained =
      calibrated < 0.42f || (margin < 0.12f && calibrated < 0.62f);
...
void EdgeAIController::getStatus(
  ...
  try {
    ...
    callback(
        ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "AI服务暂时不可用"));
  }
}
...
void EdgeAIController::getMetrics(
  ...
  try {
    ...
    nodeMetrics["avg_latency_ms"] =
        nodes.empty() ? 0.0f : totalLatency / nodes.size();
    ...
    // 双记忆RAG指标
    data["dual_memory_rag"] =
        heartlake::ai::DualMemoryRAG::getInstance().getStats();

    callback(ResponseUtil::success(data, "边缘AI性能指标获取成功"));
  } catch (const std::exception &e) {
    LOG_ERROR << "EdgeAI getMetrics error: " << e.what();
    callback(
        ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "获取性能指标失败"));
  }
}
  ...
  try {
    ...
    if (jsonPtr->isMember("prefer_onnx")) {
      ...
      if (rawPreferOnnx.isBool()) {
        ...
        std::transform(
            normalized.begin(), normalized.end(), normalized.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        preferOnnx = (normalized == "1" || normalized == "true" ||
    ...
    if (!preferOnnx && jsonPtr->isMember("analysis_mode")) {
      ...
      std::transform(
          ...
          [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
      preferOnnx = (mode == "onnx" || mode == "onnx_preferred" ||
                    mode == "high_accuracy");
    }
    ...
    if (!engine.isEnabled()) {
      callback(
          ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "边缘AI引擎未启用"));
      return;
    }
    ...
    const auto calibrated =
        calibrateConfidence(result.score, result.confidence, text.size());
    ...
    data["confidence_gap"] =
        std::max(0.0f, result.confidence - calibrated.calibratedConfidence);
    ...
    data["recommended_action"] =
        calibrated.abstained ? "ask_for_more_context" : "use_as_reference";
    ...
    callback(ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "情感分析失败"));
  }
}
...
void EdgeAIController::moderateLocal(
  ...
  try {
    ...
    for (const auto &p : result.matchedPatterns)
      data["matched_patterns"].append(p);
    ...
    if (!result.matchedPatterns.empty()) {
      reasonParts.push_back("命中: " +
                            joinStrings(result.matchedPatterns, " / "));
    }
    ...
    callback(ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "内容审核失败"));
  }
}
...
void EdgeAIController::getEmotionPulse(
  ...
  try {
    ...
    data["temperature"] = static_cast<int>(std::round(normalizedScore * 100.0));
    ...
    data["top_mood_share_percent"] =
        pulse.sampleCount > 0 ? (static_cast<double>(topMoodCount) /
                                 static_cast<double>(pulse.sampleCount)) *
                                    100.0
                              : 0.0;
    ...
    callback(
        ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "获取情绪脉搏失败"));
  }
}
...
void EdgeAIController::federatedAggregate(
  ...
  try {
    ...
    callback(
        ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "联邦学习聚合失败"));
  }
}
...
void EdgeAIController::getPrivacyBudget(
  ...
  try {
    ...
    const auto utilizationPercent =
        totalBudget.asDouble() > 0
    ...
    callback(
        ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "获取隐私预算失败"));
  }
}
  ...
  try {
    ...
    if (jsonPtr->isMember("enable_second_stage")) {
      ...
      if (v.isBool()) {
        ...
        std::transform(
            flag.begin(), flag.end(), flag.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    ...
    // 如果请求中提供了预计算向量则直接使用，否则使用真实 embedding
    // 生成查询向量。
    ...
    if (queryVec.empty()) {
      if (query.empty() || query.size() > 5000) {
        callback(ResponseUtil::badRequest(
            "query/text长度必须在1-5000之间，或提供vector"));
        return;
      }
      queryVec = heartlake::ai::AdvancedEmbeddingEngine::getInstance()
    ...
    const int candidateK =
        std::min(400, std::max(topK, topK * candidateMultiplier));
    auto results =
        engine.hnswSearch(queryVec, enableSecondStage ? candidateK : topK);
    ...
    if (!matchedStoneIds.empty()) {
      try {
        ...
        for (const auto &row : rows) {
          ...
          metadata.moodType = row["mood_type"].isNull()
                                  ? ""
                                  : row["mood_type"].as<std::string>();
          metadata.createdAt = row["created_at"].isNull()
                                   ? ""
        ...
        LOG_WARN << "EdgeAI vectorSearch metadata lookup failed: " << e.what();
      }
    }
    ...
    data["retrieval_mode"] =
        enableSecondStage ? "two_stage_ann_rerank" : "single_stage_ann";
    data["candidate_count"] =
        enableSecondStage ? candidateK : returnedCandidates;
    ...
    callback(ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "向量搜索失败"));
  }
}
...
void EdgeAIController::getAdminConfig(
  ...
  try {
    ...
    callback(ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR, "获取配置失败"));
  }
}
  ...
  try {
    ...
    for (const auto &key : body.getMemberNames()) {
      ...
      if (key == "enabled" || key == "edge_ai_enabled" ||
        ...
        persistedAiConfig["onnx_enabled"] = parseJsonStringValue(value, "auto");
        ...
        restartRequiredKeys.append(key);
                 ...
                 key == "vectorSearchEnabled" ||
                 key == "vector_search_enabled" || key == "inferenceTimeout" ||
                 key == "inference_timeout" || key == "cacheEnabled" ||
                 key == "cache_enabled" || key == "federatedEnabled" ||
                 key == "federated_enabled" || key == "quantization_bits") {
    ...
    if (appliedKeys.empty() && restartRequiredKeys.empty()) {
      callback(ResponseUtil::badRequest("没有可应用的配置项"));
      return;
    }
    ...
    const auto config = buildAdminEdgeAIConfigPayload(persistedAiConfig, stats,
                                                      engine.isEnabled());
...
bool EdgeAIController::validateEpsilon(double epsilon, double minVal,
                                       double maxVal) {
  return epsilon >= minVal && epsilon <= maxVal;
}

Json::Value EdgeAIController::buildAggregationResult(int round,
...
Json::Value EdgeAIController::collectSubsystemHealth() {
  ...
  if (!engineEnabled) {
    ...
    health["emotion_pulse"] =
        stats["emotion_window_size"].asUInt64() > 0 ? "active" : "idle";
  }
  ...
  if (!engineEnabled) {
    ...
    health["federated_learning"] =
        stats["federated_round"].asInt() > 0 ? "active" : "idle";
  }
  ...
  if (!engineEnabled) {
    ...
    health["hnsw_search"] =
        stats["hnsw_node_count"].asUInt64() > 0 ? "active" : "idle";
  }
...
Json::Value EdgeAIController::collectCacheMetrics() {
  ...
  if (totalSearches > 0 && totalNodes > 0) {
    double hitRate = std::min(1.0, static_cast<double>(totalNodes) /
                                       static_cast<double>(totalSearches));
  ...
  sc["estimated_savings"] =
      static_cast<Json::UInt64>(scStats.estimatedSavings());
...
Json::Value EdgeAIController::collectEmbeddingMetrics() {
  ...
  metrics["vector_dimension"] =
      static_cast<Json::UInt64>(engine.getHNSWVectorDimension());
  ...
  if (totalOps > 0 && totalRequests > 0) {
    metrics["throughput_per_sec"] = static_cast<double>(totalRequests) /
                                    static_cast<double>(totalOps) * 100.0;
  } else {
...
void EdgeAIController::quantizedForward(
  ...
  try {
    ...
    if (jsonPtr->isMember("input") && (*jsonPtr)["input"].isArray()) {
      for (const auto &v : (*jsonPtr)["input"])
        input.push_back(v.asFloat());
    }
      ...
      for (const auto &row : (*jsonPtr)["weights"]) {
        if (row.isArray()) {
          if (inDim == 0)
            inDim = row.size();
          for (const auto &v : row)
            weightsFlat.push_back(v.asFloat());
        }
      }
    }
...
void EdgeAIController::addNoise(
  ...
  try {
    ...
    if (jsonPtr->isMember("values") && (*jsonPtr)["values"].isArray()) {
      ...
      for (const auto &v : (*jsonPtr)["values"])
        values.push_back(v.asFloat());
...
void EdgeAIController::lakeGodChat(
  ...
  drogon::async_run([=,
                     callback = std::move(callback)]() -> drogon::Task<void> {
    ...
    try {
      ...
      auto jsonPtr = req->getJsonObject();
      if (!jsonPtr) {
        callback(ResponseUtil::error(ErrorCode::INVALID_PARAMETER,
                                     "请求体不能为空"));
        co_return;
      }
      ...
      if (!moderateResult.passed) {
        callback(ResponseUtil::error(ErrorCode::CONTENT_DELETED,
                                     "内容未通过审核: " +
                                         moderateResult.suggestion));
        co_return;
      }

      // 情感分析
      auto sentiment = engine.analyzeSentimentLocal(content);
      engine.submitEmotionSample(sentiment.score, sentiment.mood,
                                 sentiment.confidence);

      // 生成湖神回复
      auto &rag = heartlake::ai::DualMemoryRAG::getInstance();
      auto ragReply = rag.generateResponseResult(
          userId, content, sentiment.mood, sentiment.score);

      // 异步存入数据库
      auto db = drogon::app().getDbClient("default");
      db->execSqlAsync(
          "INSERT INTO lake_god_messages (user_id, role, content, mood, "
          "emotion_score) VALUES ($1, $2, $3, $4, $5)",
          [](const drogon::orm::Result &) {},
          [](const drogon::orm::DrogonDbException &e) {
            LOG_ERROR << "Insert lake_god_messages (user) failed: "
                      << e.base().what();
          },
      ...
      db->execSqlAsync(
          ...
          [](const drogon::orm::Result &) {},
          [](const drogon::orm::DrogonDbException &e) {
            LOG_ERROR << "Insert lake_god_messages (assistant) failed: "
                      << e.base().what();
          },
      ...
      callback(ResponseUtil::success(
          data, ragReply.degraded ? "湖神本地陪伴回复" : "湖神回复成功"));
    } catch (const std::exception &e) {
...
void EdgeAIController::lakeGodHistory(
  ...
  try {
    ...
    db->execSqlAsync(
        "SELECT role, content, mood, emotion_score, created_at FROM "
        "lake_god_messages WHERE user_id = $1 ORDER BY created_at ASC LIMIT 50",
        [cb](const drogon::orm::Result &result) {
          ...
          for (const auto &row : result) {
            ...
            msg["mood"] =
                row["mood"].isNull() ? "" : row["mood"].as<std::string>();
            msg["emotion_score"] = row["emotion_score"].isNull()
                                       ? 0.0
        ...
        [cb](const drogon::orm::DrogonDbException &e) {
          ...
          (*cb)(ResponseUtil::error(ErrorCode::AI_SERVICE_ERROR,
                                    "历史消息查询失败"));
        },
        userId);
  } catch (const std::exception &e) {
