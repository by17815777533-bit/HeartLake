...
void InteractionController::createConnection(
  ...
  try {
    ...
    if (targetUserId.empty()) {
      targetUserId = (*json).get("targetUserId", "").asString();
    }
    if (targetUserId.empty()) {
      targetUserId = (*json).get("user_id", "").asString();
    }
    if (targetUserId.empty()) {
...
void InteractionController::getConnectionMessages(
  ...
  try {
    auto [page, pageSize] = safePagination(req);
    if (pageSize > 50)
      pageSize = 50; // 消息列表限制更严格

    auto service = getInteractionService();
    auto result =
        service->getConnectionMessages(connectionId, userId, page, pageSize);
...
void InteractionController::createConnectionMessage(
  ...
  try {
    ...
    auto service = getInteractionService();
    auto result =
        service->createConnectionMessage(connectionId, userId, content);
