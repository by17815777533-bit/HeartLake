...
- [本地启动与运行手册（非 Docker）](docs/01_本地启动与运行手册.md)
- [API 与实时链路手册](docs/02_API与实时链路手册.md)
- [端到端测试与故障排查手册](docs/03_端到端测试与故障排查手册.md)
...
| 测试 | GTest + Vitest + Flutter Test | -- | 后端 55 套件 1400+ 用例 + 管理后台 25 套件 + Flutter 1344 用例 |
...
| EdgeAI 子系统数 | 8 个 |
| 通知类型数 | 8 种 |
| 后端测试（GTest） | 55 个可执行文件，635+ 用例，100% 通过 |
| 前端测试（Vitest） | 24 个测试文件，582 用例，100% 通过 |
| 世界级验证门控 | 11/11 通过，得分 1.0 |
| 情感分析准确率 | 极性 96.88%，宏平均 F1 97.69% |
| HNSW 搜索延迟 | 44.98 μs/query，Recall@10 = 0.894 |
| 内容审核检出率 | 100%（AC 自动机） |
