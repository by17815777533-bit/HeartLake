...
- 准备上线先看：[docs/10_上线检查清单.md](./docs/10_上线检查清单.md)
- 2C2G Ubuntu 部署：[docs/deploy-ubuntu-2c2g.md](./docs/deploy-ubuntu-2c2g.md)
- API 与实时链路真源：[docs/02_API与实时链路手册.md](./docs/02_API与实时链路手册.md)
...
- 临时好友只由石头/纸船互动建立；手动直连和手动升级永久好友不可用。
- 客户端只认完整持久化会话；验证失败不会直接进入首页。
- 管理端和移动端列表页要求显式 `total/page/page_size/has_more` 集合契约。
- 情绪日历、热力图、情绪脉搏只消费真实 `score/raw_score/mood` 数据。
- 高级推荐只消费真实高级算法结果，不回退成普通榜单或空成功。
- Edge AI 管理配置对非法值直接拒绝。
- 移动端发布版默认连接当前生产网关 `http://121.41.195.165`；若要切换发布目标，构建时显式传 `PUBLIC_ORIGIN`。

## 一致性约束

- 移动端启动页只有在会话验证成功后才进入首页；刷新令牌失败时会停留在启动页并要求重试或重新登录，不再把“未验证会话”伪装成已登录状态。路由守卫只认持久化且完整的会话（`token + user_id`），不再把内存态残缺会话当成已登录。
- 隐私设置与推荐模块在请求失败时会显式提示并保留已确认状态，不再把失败默认为空数据、已保存或推荐缺失。
- 情绪日历、热力图与情绪脉搏链只消费真实 `score/raw_score/mood` 数据；坏 payload 不再落地成空图、默认中性值或“看起来成功”的记录。
- 管理后台 Dashboard 在接口失败时会展示“部分数据未刷新”告警，并继续展示最近一次成功结果；“高级热门内容”只消费推荐接口结果，不再回退成普通热度榜冒充高级算法输出。主布局实时状态与 WebSocket 异常也必须显式暴露，不再只打日志。
...
| CI/CD | 仓库内未内置自动流水线 | 当前以手工脚本构建、校验与发布为主 |
...
| RecommendationController | `/api/recommendations` | 个性化推荐、热门内容、趋势、高级共鸣推荐；管理端调试入口为 `/api/admin/recommendations/advanced` |
...
| 文档 | 内容 |
|------|------|
| [文档地图](docs/README.md) | 统一入口、阅读顺序、上线前必读文档 |
...
| [Ubuntu 2C2G Lite 部署](docs/deploy-ubuntu-2c2g.md) | 低配云服务器部署、环境生成、增量发布与冒烟验证 |
| [上线检查清单](docs/10_上线检查清单.md) | 正式上线前的配置、校验、部署、回滚检查 |
