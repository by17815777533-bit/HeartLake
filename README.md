...
- 移动端启动页只有在会话验证成功后才进入首页；刷新令牌失败时会停留在启动页并要求重试或重新登录，不再把“未验证会话”伪装成已登录状态。
- 隐私设置与推荐模块在请求失败时会显式提示并保留已确认状态，不再把失败默认为空数据、已保存或推荐缺失。
- 管理后台 Dashboard 在接口失败时会展示“部分数据未刷新”告警，并继续展示最近一次成功结果；“高级热门内容”只消费推荐接口结果，不再回退成普通热度榜冒充高级算法输出。
