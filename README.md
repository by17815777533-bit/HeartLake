...
三端测试共计 112 个测试文件，覆盖单元测试、集成测试、压力测试和端到端冒烟测试。

| 模块 | 框架 | 测试文件数 | 运行命令 |
|------|------|-----------|----------|
| 后端 | Google Test | 55 | `cd backend/build && cmake .. && make -j$(nproc) && ctest --output-on-failure -j8` |
| 移动端 | Dart test | 33 | `cd frontend && flutter test` |
| 管理后台 | Vitest | 24 | `cd admin && npx vitest run` |
